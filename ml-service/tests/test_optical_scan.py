...
def _build_synthetic_optical_form(answers: list[str], width: int = 900, height: int = 1400) -> bytes:
    ...
    for row_index, selected in enumerate(answers):
        for col_index, option in enumerate(["A", "B", "C", "D", "E"]):
            ...
            if selected == option:
                cv2.circle(image, (center_x, center_y), max(4, radius - 6), 0, -1)
    ...
    warped = cv2.warpPerspective(image, transform, (width, height), borderValue=255)
